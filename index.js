...
Requires HomeKit Bridge Indigo plug-in: https://github.com/Colorado4Wheeler/HomeKit-Bridge
See http://forums.indigodomo.com/viewforum.php?f=201 for installation instructions
...
Fields:
    ...
    "port": Port number of your HomeKit Bridge API server (required)
...
var CommunityTypesGen = require('hap-nodejs-community-types');
var Service, Characteristic, Accessory, uuid, CommunityTypes;
...
module.exports = function(homebridge) {
    ...
    CommunityTypes = CommunityTypesGen(homebridge);
...
// Adds an Indigo2Accessory object to this.foundAccessories and this.accessoryMap
...
// Creates an Indigo2Accessory object from a JSON device description
// Returns an Indigo2Accessory object, or null if error
// item: JSON describing the device
Indigo2Platform.prototype.createAccessory = function(item) {
    ...
    if (! service) {
        service = CommunityTypes[serviceName];
        if (! service) {
...
// Makes a request to the HomeKit Bridge API
...
Indigo2Platform.prototype.indigoRequest = function(path, method, qs, callback) {
    ...
    // All requests to HomeKit Bridge are serialized, so that there is no more than one outstanding request at a time
    this.requestQueue.push(options, callback);
};

// Makes a request to HomeKit Bridge API and parses the JSON response
// path: the path of the request, relative to the base URL in the configuration, starting with a /
// method: the type of HTTP request to make (e.g. GET, POST, etc.)
// qs: the query string to include in the request (optional)
...
// If the ID corresponds to an accessory, invokes updateCharacteristicValues() on that accessory with the
// hkcharacteristics array from the POST body content (JSON)
// If the POST body does not contain a hkcharacteristics array, invokes refresh() on that accessory instead
// Sends a 200 HTTP response if successful, a 404 if the ID is not found, or a 500 if there is an error
...
// deviceURL: the path of the HomeKit Bridge API call for this device, starting with a /
...
function Indigo2Accessory(platform, serviceType, deviceURL, id, objectType, name, accessoryJSON, characteristicsJSON) {
    ...
    if (this.debug) {
        this.log("Creating Accessory from JSON:\n%s", JSON.stringify(accessoryJSON));
    }
...
Indigo2Accessory.prototype.addCharacteristic = function(characteristicJSON) {
    ...
    if (! characteristicClass) {
        characteristicClass = CommunityTypes[name];
        if (! characteristicClass) {
    ...
    return function(value, callback, context) {
        ...
        if (cachedCharacteristicValue) {
            ...
            if (value !== oldValue || this.platform.sendDuplicateUpdates) {
                ...
                if (this.debug) {
                    this.log("%s: set(%s) %s - ignored unchanged value", this.name, characteristicName, value);
                }
                if (callback) {
                    callback();
                }
...
// characteristicsJSON: JSON array of characteristics from the HomeKit Bridge API, each with name and value attributes
...
// characteristicJSON: JSON dictionary from the HomeKit Bridge API, with required name and value attributes
    ...
    if (name) {
        ...
        if (cachedCharacteristicValue) {
            ...
            if (newValue === undefined || newValue === null) {
                ...
                if (this.debug) {
                    this.log("%s: updateCharacteristicValue(%s): %s - ignored unchanged value", this.name, name, newValue);
                }
            }
        }
        else {
...
// Calls the HomeKit Bridge API to get the latest state for this Accessory, and updates the Accessory's properties to match
...
// Calls the HomeKit Bridge API to get the latest state of this Accessory, and updates the Accessory's properties to match,
